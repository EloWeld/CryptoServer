...
def add_journal(data):
    ...
    # Оставляем только последние 200 строк
...
def on_message(ws, message):
    ...
    with lock:
        ...
        if len(price_history[symbol]) > N:
            ...
            if change_amount >= M:
                print(f"{symbol} price PUMPED by {change_amount:.2f}% over the last {N} minutes; Datetime: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                ...
                try:
                    ...
                    print(f"Error during journal append: {e}, {traceback.format_exc()}")
                
            if change_amount * -1 >= M:
                print(f"{symbol} price DUMPED by {change_amount:.2f}% over the last {N} minutes Datetime: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
if __name__ == "__main__":
    ...
    for i in range(0, num_pairs, max_pairs_per_ws):
        ...
        print(i, i+ max_pairs_per_ws, len(pairs))
